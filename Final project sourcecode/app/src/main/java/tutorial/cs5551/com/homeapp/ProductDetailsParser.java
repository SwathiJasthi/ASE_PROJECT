...
public class ProductDetailsParser {
    static public class ProductDataParser {
       ...
       /*     String IMAGE_BASE_URL = "https://maps.googleapis.com/maps/api/place/photo?maxwidth=100&maxheight=100&photoreference=";
            ...
            if (dataobject.has("offers")) {

                JSONArray photosarray = new JSONArray(dataobject.getString("offers"));
                String merchantnames[] = new String[photosarray.length()];
                Double prices[] = new Double[photosarray.length()];
                String websiteurl [] = new String[photosarray.length()];
                ...
                for (int j = 0; j < photosarray.length(); j++) {
                    ...
                    merchantnames[j] = photoobject.getString("merchant");
                    prices[j] = photoobject.getDouble("price");
                    websiteurl[j] = photoobject.getString("link");
                }
                productDetails.setMerchantnames(merchantnames);
                productDetails.setPrices(prices);
                productDetails.setWebsitelink(websiteurl);
                // PHOTO_REF_URL = ref_value;

            } else {
                String merchant[] = new String[1];
                Double price[] = new Double[1];
                merchant[0] = "N/A";
                price[0] = 0.0;
                productDetails.setMerchantnames(merchant);
                productDetails.setPrices(price);
            }
